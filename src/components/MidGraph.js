...
export default function Graph() {
  ...
  useEffect(() => {
    var time = localStorage.getItem("mt");
    var diff = 25;
    if (time) diff = timeDiff(time);
    console.log("diff: " + diff);
    if (diff >= 6) {
      localStorage.setItem("mt", moment(new Date()));
      ...
      validate
        .then((data) => {
          ...
          // eslint-disable-next-line
          midarea = riskarea.mid;
          // eslint-disable-next-line
          szMidarea = midarea.filter((area) => area.includes("深圳市"));
          localStorage.setItem("midarea", JSON.stringify(szMidarea));
      ...
      szMidarea = JSON.parse(localStorage.getItem("midarea"));
      setArea(szMidarea);
    }
  }, []);
